...
ZEN_HOME="$HOME/.config/zen/profiles"
...
backup_chrome_css() {
  ...
  if [ "$flavor" == "firefox" ]; then
    ...
    mkdir -p "$backup_dir"
    find $profile_chrome_dir -maxdepth 1 -name "*.css" -not -name "zen-themes.css" -exec mv {} $backup_dir/ \;
  else
...
chrome_css_setup() {
  ...
  if [ "$flavor" == "zen" ]; then
    ...
    if [ ! -e "$profile_chrome_dir/userChrome.css" ]; then
      pushd "$CONFIG_HOME/chrome/zen" >/dev/null || exit
      cp -f *.css "$profile_chrome_dir"
      popd >/dev/null || exit
    fi
  fi
}
...
while [ "$#" -gt 0 ]; do
  ...
  -zen-all)
    ...
    config_profile "zen" "coding" "coding" &&
